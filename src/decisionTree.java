...
class treeNode{
	...
	this.informationGain=node.informationGain;
	...
	void dataInexCount(HashMap<Integer,HashMap<String,Integer>> records,String attr, int value){
	...
	void computeInfoGain(){
		...
		if(left==null && right==null){
			...
			informationGain=(double)entropy-(product1+product2);
...
class decisionTree{
	...
	double buildAndCalculateIG(instanceDB instance, String attrVal,treeNode node){
		...
		left.dataInexCount(instance.getRecords(), attrVal,0);
		...
		right.dataInexCount(instance.getRecords(), attrVal,1);
		...
		left.dataInexCount(trainingSet.getRecords(), left.getAttributeValue(attrVal), 0);
		...
		right.dataInexCount(trainingSet.getRecords(), right.getAttributeValue(attrVal), 1);
...
public static void main(String[] args) throws IOException{
	...
	if(args.length>=6){
	 ...
	 try{
			if(args[0].length()>0){
				...
				System.out.println("Enter Input:<L>");
				return;
			}
			...
			System.out.println("Input:<L> and <K>");
			return;
		}		
		...
		if(args[3].length()>0){
			...
			System.out.println("Input:<validation-set>");
			return;
		}
		...
		if(args[4].length()>0){
			...
			System.out.println("Input:<test-set>");
			return;
		}
		...
		if(args[5].length()>0){
			...
			System.out.println("Input:<to-print>");
			return;
		}
	...
	System.out.println("Decision Tree : Training set<"+trainSet+">");	
	...
	System.out.println("Accuracy : Test Set<"+testSet+"> pre pruning="+accuracy+"%");
	...
	System.out.println("Accuracy : Test Set<"+testSet+"> ="+accuracy+"%");
